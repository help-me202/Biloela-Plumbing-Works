...
const products = [
  { id: 1, name: 'Gas Bottle', size: '9kg', basePrice: 60.0, sizeType: 'small' },
  { id: 2, name: 'Gas Bottle', size: '14kg', basePrice: 75.0, sizeType: 'large' }
];
...
function getInventory(productId, zoneId) {
  const item = inventory.find(record => record.productId === productId && record.zoneId === zoneId);
  return item ? item.qty : 0;
}

function computePrice(product, zone, collection = 'delivery') {
  const zonePrice = zonePrices.find(r => r.productId === product.id && r.zoneId === zone.id);
  const base = zonePrice && typeof zonePrice.overridePrice === 'number' ? zonePrice.overridePrice : product.basePrice;
...
app.get('/api/zones', (req, res) => {
  res.json(zones.map(zone => ({ id: zone.id, name: zone.name, deliveryFee: zone.deliveryFee })));
});
...
app.get('/api/price', (req, res) => {
  ...
  const zonePrice = zonePrices.find(r => r.productId === product.id && r.zoneId === zone.id);
  const basePrice = zonePrice && typeof zonePrice.overridePrice === 'number' ? zonePrice.overridePrice : product.basePrice;
...
app.post('/api/reserve', (req, res) => {
  ...
  if (!size || !postcode || !quantity) {
    return res.status(400).json({ error: 'size, postcode and quantity are required' });
  }

  const product = getProduct(size);
  if (!product) return res.status(404).json({ error: 'Product size not found' });

  const zone = findZone(postcode);
  const stock = getInventory(product.id, zone.id);
  if (quantity > stock) {
    return res.status(409).json({ error: 'Insufficient stock', available: stock });
  }

  const record = inventory.find(item => item.productId === product.id && item.zoneId === zone.id);
  if (record) record.qty = record.qty - quantity;

  res.json({ success: true, reserved: quantity, remaining: getInventory(product.id, zone.id) });
});
